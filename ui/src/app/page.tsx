  ...
  return (
    <div className="max-w-6xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl flex items-center gap-4">
          <div className={`p-4 rounded-xl ${balance >= 0 ? 'bg-blue-500/10 text-blue-500' : 'bg-red-500/10 text-red-500'}`}>
            <Wallet size={32} />
          </div>
          <div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
        ...
        {transactions.length === 0 ? (
          ...
          <div className="divide-y divide-slate-800">
            {transactions.map((t) => (
              <div key={t.id} className="p-6 flex items-center justify-between hover:bg-slate-800/50 transition-colors">
                <div className="flex flex-col gap-1">
                  <span className="font-medium text-slate-200">{t.description}</span>
                  <div className="flex items-center gap-3 text-sm text-slate-400">
                    <span className="bg-slate-800 px-2 py-0.5 rounded text-xs">{t.categoryName || 'General'}</span>
